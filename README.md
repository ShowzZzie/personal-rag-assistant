...
- **`mypy --strict` doesn't understand `pydantic-settings`.** `Settings()` in `config.py` is instantiated with no arguments even though `openai_api_key` and `anthropic_api_key` have no defaults — `pydantic-settings` populates them from the environment at runtime, but mypy has no way to know that without the `pydantic.mypy` plugin enabled in `pyproject.toml`, so `mypy --strict src/` currently reports two `call-arg` errors on that line.
